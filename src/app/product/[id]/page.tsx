...
import { ShoppingBag, Star, Heart, ArrowLeft, Plus, Minus, Sparkles, MessageCircle, Check, Package, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function ProductDetailPage() {
  ...
  const { unitPrice, appliedTier, savings } = calculateSmartUnitPrice(
    product.basePrice,
    quantity,
    product.comboTiers
  );
  ...
  const validateVariantSelection = (): boolean => {
    if (product.variants && product.variants.length > 0 && !selectedVariant) {
      ...
      setWarningToast('⚠️ Vui lòng bấm chọn Phân loại (Màu sắc / Mẫu) trước khi đặt hàng nhé! 🌸');
      setTimeout(() => setWarningToast(null), 3200);
  ...
  const handleAddToCart = () => {
    ...
    addItem(product, finalQty, selectedVariant, customNote);
  ...
  const totalStockCount = product.variants && product.variants.length > 0
    ? product.variants.reduce((acc, v) => acc + (v.stock || 0), 0)
    : (product.stock || 0);

  const maxAvailable = selectedVariant?.stock !== undefined 
  ...
  return (
    <div className="py-4 sm:py-6 space-y-6 max-w-5xl mx-auto pb-24 md:pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 sm:gap-8 items-start">
              ...
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`w-16 h-16 sm:w-20 sm:h-20 rounded-2xl overflow-hidden border-2 flex-shrink-0 transition ${
                    selectedImage === img ? 'border-rose-500 shadow-sm scale-105' : 'border-pink-100 opacity-70 hover:opacity-100'
                  }`}
                >
                  <img
                    src={img}
        ...
        {/* Right Column: Product Config & Buy Form */}
        <div className="md:col-span-6 space-y-5">
          ...
          <div>
            ...
            <h1 className="text-xl sm:text-3xl font-black text-gray-800 mt-2 leading-snug">
              {product.name}
            </h1>
            ...
            <div className="flex items-center flex-wrap gap-2.5 mt-2.5 text-xs text-gray-500">
              ...
              <span className="text-gray-600 font-medium">Đã bán: <strong className="text-gray-800 font-bold">{product.soldCount}</strong></span>
              <span className="text-gray-300">•</span>
              {/* Tình trạng tồn kho tổng nổi bật */}
              {totalStockCount > 0 ? (
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-bold text-xs shadow-2xs">
                  ...
                  <span>Sẵn hàng trong kho: <strong className="text-emerald-800 font-black">{totalStockCount.toLocaleString('vi-VN')}</strong> cái</span>
                </div>
              ) : (
          ...
          <div className="bg-rose-50/60 p-4 rounded-2xl border border-rose-100 flex items-baseline gap-3">
            <span className="text-2xl sm:text-3xl font-black text-rose-600">{formatVND(unitPrice)}</span>
            <span className="text-xs text-gray-400 font-medium">/cái</span>
          ...
          {/* Smart Pricing Bar with Combo Tiers (50, 100, 200 pcs...) */}
          {product.comboTiers && product.comboTiers.length > 0 && (
            <SmartPricingBar
              ...
              onQuantityChange={(qty) => {
                if (selectedVariant && maxAvailable > 0 && qty > maxAvailable) {
                  setQuantity(maxAvailable);
                  setWarningToast(`Kho chỉ còn ${maxAvailable} cái cho phân loại ${selectedVariant.name}.`);
          ...
          {product.variants && product.variants.length > 0 && (
            <div
              ...
              className={`p-3.5 sm:p-4 rounded-2xl transition-all duration-300 ${
                variantError
                  ? 'bg-rose-50/70 border-2 border-rose-500 ring-4 ring-rose-200 shadow-md'
                  : 'bg-pink-50/30 border border-pink-100'
              }`}
            >
              <div className="flex items-start gap-3 sm:gap-4">
                <span className="text-xs text-gray-500 font-bold min-w-[70px] pt-1.5">Phân Loại:</span>
                <div className="flex-1 space-y-2">
                  <div className="flex flex-wrap gap-2">
                    {product.variants.map((v) => {
                      ...
                      // Out of stock (Shopee style dimmed, disabled, line-through)
                      if (isOutOfStock) {
                        return (
                          <div
                            key={v.id}
                            title={`Mẫu "${v.name}" tạm thời hết hàng`}
                            className="px-3 py-1.5 rounded-lg border border-dashed border-gray-300 bg-gray-100/80 text-gray-400 text-xs flex items-center gap-1.5 opacity-45 line-through select-none cursor-not-allowed"
                          >
                            {v.colorHex && (
                              <span
                                className="w-3.5 h-3.5 rounded-full border border-gray-300 shrink-0 filter grayscale"
                      ...
                      // Clickable Chip (Shopee signature selected corner checkmark)
                      return (
                        <button
                          key={v.id}
                          type="button"
                          ...
                          className={`relative px-3.5 py-1.5 rounded-lg border text-xs font-bold transition-all flex items-center gap-1.5 select-none overflow-hidden ${
                            isSelected
                              ? 'border-2 border-rose-500 text-rose-600 bg-white font-black shadow-xs'
                              : 'border-gray-200 bg-white text-gray-700 hover:border-rose-400 hover:text-rose-600 hover:bg-rose-50/40'
                          }`}
                        >
                          {v.colorHex && (
                            <span
                              className={`w-3.5 h-3.5 rounded-full border shrink-0 transition-transform ${
                                isSelected ? 'border-rose-400 scale-110' : 'border-black/10'
                              }`}
                              style={{ backgroundColor: v.colorHex }}
                            />
                          )}
                          ...
                          {isSelected && (
                            <div className="absolute bottom-0 right-0 w-3 h-3 overflow-hidden pointer-events-none">
                              <div className="absolute bottom-0 right-0 w-0 h-0 border-solid border-t-transparent border-l-transparent border-b-rose-500 border-r-rose-500 border-t-0 border-l-0 border-b-[12px] border-r-[12px]" />
                              <svg
                  ...
                  {/* Selected indicator / Validation warning */}
                  {selectedVariant ? (
                    <p className="text-[11px] text-gray-500 pt-0.5">
                      Đã chọn: <strong className="text-rose-600 font-bold">{selectedVariant.name}</strong> • Còn <strong className="text-emerald-700 font-bold">{selectedVariant.stock?.toLocaleString('vi-VN')}</strong> cái
                    </p>
                  ) : variantError ? (
                    <p className="text-xs text-rose-600 font-bold flex items-center gap-1 animate-pulse pt-0.5">
                      <AlertCircle className="w-3.5 h-3.5" />
          ...
          {/* Quantity Selector & Quick Selector (SHOPEE STYLE) */}
          <div className="space-y-2">
              ...
              <div className="flex items-center">
                <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden bg-white shadow-2xs">
                  <button
                    ...
                    className="w-8 h-8 flex items-center justify-center text-gray-600 hover:bg-gray-100 hover:text-rose-600 border-r border-gray-200 transition disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                  ...
                  <button
                    ...
                    className="w-8 h-8 flex items-center justify-center text-gray-600 hover:bg-gray-100 hover:text-rose-600 border-l border-gray-200 transition disabled:opacity-40 disabled:cursor-not-allowed"
                  >
            ...
            {/* Quick Quantity bump buttons */}
            <div className="flex items-center gap-1.5 pl-0 sm:pl-[84px] text-xs text-gray-500">
              ...
              {[10, 50, 100, 200].map((num) => (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (selectedVariant && maxAvailable > 0 && num > maxAvailable) {
                      setQuantity(maxAvailable);
                      setWarningToast(`Kho chỉ còn tối đa ${maxAvailable} cái cho phân loại này.`);
                      setTimeout(() => setWarningToast(null), 2500);
                    } else {
                      setQuantity(num);
                    }
                  ...
                  className={`px-2 py-0.5 rounded-md text-[11px] font-bold border transition ${
                    quantity === num
                      ...
                      : 'bg-white hover:bg-rose-50 text-gray-600 hover:text-rose-600 border-gray-200'
                  }`}
                >
                  +{num}
                </button>
          ...
          {/* Custom Note (Shopee style inline input) */}
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <input
              ...
              className="flex-1 px-3 py-2 text-xs bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-300 text-gray-700 transition"
            />
          </div>
          ...
          <div className="hidden md:flex items-center gap-3 pt-2">
            <button
              ...
              className="min-w-[190px] h-12 px-6 rounded-xl border border-rose-500 bg-rose-50/80 hover:bg-rose-100 text-rose-600 font-extrabold text-sm flex items-center justify-center gap-2 shadow-2xs transition transform active:scale-98"
            >
            ...
            <button
              ...
              className="min-w-[170px] h-12 px-8 rounded-xl bg-gradient-to-r from-red-500 via-rose-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white font-black text-sm flex items-center justify-center gap-2 shadow-lg shadow-rose-200 transition transform active:scale-98"
            >
              <span>Mua Ngay</span>
            </button>
          </div>
          ...
          <div className="pt-4 border-t border-pink-100 space-y-3 text-xs text-gray-600 leading-relaxed">
            ...
            <div className="grid grid-cols-2 gap-2 bg-pink-50/40 p-3 rounded-2xl border border-pink-100 text-[11px]">
              ...
              <div className="bg-white/80 p-2 rounded-xl border border-pink-100">
                <span className="text-gray-500 block font-medium">📦 Tồn kho sẵn xưởng:</span>
                <strong className="text-emerald-700 font-extrabold text-xs">{(product.stock || 0).toLocaleString('vi-VN')} sản phẩm</strong>
              </div>
            </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-gray-200 shadow-2xl flex items-stretch h-14">
        ...
        <button
          ...
          className="relative flex flex-col items-center justify-center px-3.5 text-[10px] text-gray-600 hover:text-rose-600 border-r border-gray-100 shrink-0"
        >
        ...
        <button
          ...
          className="flex-1 bg-rose-50/90 hover:bg-rose-100 text-rose-600 font-bold text-xs flex flex-col items-center justify-center active:bg-rose-100 transition px-2"
        >
        ...
        <button
          ...
          className="flex-1 bg-gradient-to-r from-red-500 to-rose-600 text-white font-black text-xs flex flex-col items-center justify-center active:brightness-95 transition px-2 shadow-sm"
        >
          <span>Mua Ngay</span>
      ...
      {/* SHOPEE MOBILE BOTTOM SHEET DRAWER */}
      {isMobileSheetOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-end animate-fade-in">
          ...
          <div className="relative w-full bg-white rounded-t-3xl max-h-[85vh] flex flex-col z-10 shadow-2xl animate-slide-up">
            {/* Header: Product Preview & Close Button */}
            <div className="p-4 border-b border-gray-100 flex items-start gap-3 relative">
              <img
                src={selectedImage}
                alt={product.name}
                className="w-20 h-20 rounded-xl object-cover border border-gray-200 shrink-0 shadow-2xs"
              />
              <div className="flex-1 min-w-0 pr-8">
                <p className="text-lg font-black text-rose-600">{formatVND(unitPrice)}</p>
                <p className="text-xs text-gray-500 mt-0.5">
                  Kho: <strong className="text-gray-800 font-bold">{maxAvailable.toLocaleString('vi-VN')}</strong>
                </p>
                <p className="text-xs text-gray-600 mt-1 truncate">
                  {selectedVariant ? (
                    <span>Đã chọn: <strong className="text-rose-600 font-bold">{selectedVariant.name}</strong></span>
                  ) : (
                    <span className="text-rose-500 font-medium">Vui lòng chọn phân loại</span>
                  )}
                </p>
              </div>
              <button
                ...
                className="absolute top-3.5 right-3.5 w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-500 text-sm font-bold"
              >
                ✕
              </button>
            </div>
            ...
            <div className="p-4 space-y-4 overflow-y-auto max-h-[50vh]">
              {product.variants && product.variants.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-xs font-bold text-gray-700">Màu Sắc / Phân Loại:</h4>
                  <div className="flex flex-wrap gap-2">
                    {product.variants.map((v) => {
                      const isSelected = selectedVariant?.id === v.id;
                      const vStock = v.stock ?? 0;
                      ...
                      if (isOutOfStock) {
                        return (
                          ...
                          >
                            {v.colorHex && (
                              <span className="w-3.5 h-3.5 rounded-full border border-gray-300 shrink-0 filter grayscale" style={{ backgroundColor: v.colorHex }} />
                            )}
                            <span>{v.name}</span>
                          </div>
                        );
                      ...
                      return (
                        <button
                          ...
                          className={`relative px-3.5 py-1.5 rounded-lg border text-xs font-bold flex items-center gap-1.5 select-none overflow-hidden ${
                            isSelected
                              ? 'border-2 border-rose-500 text-rose-600 bg-white font-black'
                              : 'border-gray-200 bg-white text-gray-700 active:bg-rose-50'
                          }`}
                        >
                          {v.colorHex && (
                            <span className="w-3.5 h-3.5 rounded-full border border-black/10 shrink-0" style={{ backgroundColor: v.colorHex }} />
                          )}
                          <span>{v.name}</span>
                          ...
                          {isSelected && (
                            <div className="absolute bottom-0 right-0 w-3 h-3 overflow-hidden pointer-events-none">
                              <div className="absolute bottom-0 right-0 w-0 h-0 border-solid border-t-transparent border-l-transparent border-b-rose-500 border-r-rose-500 border-t-0 border-l-0 border-b-[12px] border-r-[12px]" />
                              <svg className="absolute bottom-[0.5px] right-[0.5px] w-2 h-2 text-white" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2.5">
                                <path d="M2 6l3 3 5-5" />
                              </svg>
                            </div>
                          )}
              ...
              <div className="space-y-1.5 pt-2 border-t border-gray-100">
                ...
                <input
                  ...
                  value={customNote}
                  onChange={(e) => setCustomNote(e.target.value)}
                  className="w-full px-3 py-2 text-xs bg-white border border-gray-200 rounded-lg focus:outline-none focus:border-rose-400"
                />
              </div>
            </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-white">
              <button
                ...
                className={`w-full py-3.5 rounded-xl font-black text-sm flex items-center justify-center gap-2 shadow-lg transition active:scale-98 ${
                  mobileSheetAction === 'buy'
                    ? 'bg-gradient-to-r from-red-500 via-rose-500 to-pink-500 text-white shadow-rose-200'
